@if ($action == 'hapus')
<form action="{{ route('pengumuman-admin.destroy', ['pengumuman_admin' => Crypt::encrypt($data->id)]) }}" method="post">
    @csrf
    @method('DELETE')
...
<div class="modal fade show" id="edit_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
        ...
        <form method="POST" action="{{ route('pengumuman-admin.update', ['pengumuman_admin' => Crypt::encrypt($data->id)]) }}">
            @csrf
            @method('PUT')
            @csrf
